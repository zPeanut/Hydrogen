...
public class ElementComboBox extends Element {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		Utils.drawRect(x, y + 14, x + width, y + 15, 0x77000000);
		if (comboextended) {
			Utils.drawRect(x, y + 15, x + width, y + height, 0xaa121212);
			double ay = y + 15;
			for (String sld : set.getOptions()) {
				...
				if (sld.equalsIgnoreCase(set.getValString())) {
					Utils.drawRect(x, ay, x + 1.5, ay + FontUtil.getFontHeight() + 2, clr1);
				}
				/*
				...
				if (mouseX >= x && mouseX <= x + width && mouseY >= ay && mouseY < ay + FontUtil.getFontHeight() + 2) {
					Utils.drawRect(x + width - 1.2, ay, x + width, ay + FontUtil.getFontHeight() + 2, clr2);
				}
